...
func ReadOnCurrentPidNs(iter *link.Iter) ([]byte, error) {
	var b bytes.Buffer
	err := WriteOnCurrentPidNs(iter, &b)
	if err != nil {
		return nil, err
	}
	return b.Bytes(), nil
...
func DiscardOnCurrentPidNs(iter *link.Iter) error {
	return WriteOnCurrentPidNs(iter, io.Discard)
}

// WriteOnCurrentPidNs reads the iterator in the current pid namespace and writes it to w.
func WriteOnCurrentPidNs(iter *link.Iter, w io.Writer) error {
	file, err := iter.Open()
	if err != nil {
